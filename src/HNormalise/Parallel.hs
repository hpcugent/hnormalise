...
module HNormalise.Parallel
  ( conduitPooledMapBuffered
  ) where
...
--
-- To cope with this, `conduitPooledMapBuffered workerOutputBufferSize f` gives each
...
conduitPooledMapBuffered workerOutputBufferSize f = do
  ...
  bracketP
    spawnWorkers
    Async.cancel -- TODO switch to e.g. `liftBase UnliftedAsync.uninterruptibleCancel` once we have async-2.1.1
    $ \workersAsync -> do

      let mustBeNonempty = fromMaybe (error "conduitPooledMapBuffered: outQueue cannot be empty")
      ...
      let loop :: Int -> Int -> Conduit a m b
          loop numWorkersRampedUp numInQueue =
            ...
            await >>= \case
              Nothing -> do -- upstream conduit is done, tell all workers to finish
                ...
                for_ [1..(numCaps - numWorkersRampedUp)] $ \_ -> -- need to quit workers that were never ramped up too
                  putInVar Nothing
                let numInQueueAfterStopping = numInQueue - numWorkersRampedUp
                for_ [1..numInQueueAfterStopping] $ const yieldQueueHead
              ...
              Just a
                ...
                | otherwise -> do
                    ...
                    let popAsManyAsPossible !remainingInQueue
                          | remainingInQueue < numWorkersRampedUp = error "conduitPooledMapBuffered: remainingInQueue < numWorkersRampedUp"
